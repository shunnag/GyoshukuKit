...
- 対応: ZIP / ZIP64 の新規作成、stored / raw deflate (system zlib)
- 依存: 開発中は隣接する `../KaitoKit`。現在はテストの往復検証に使用
- ライセンス: MIT
...
既存書庫の追加・削除・改名、暗号化、macOS metadata の保存、他形式の作成は
今後の段階です。[設計書](Documentation/design.md)と
[検証記録](Documentation/verification/2026-09-10-zip-writer.md)を参照してください。
...
65,536 entry の全件検証を含めます。展開中は約 4 GiB の空き領域を必要とし、
...
> The local `../KaitoKit` dependency currently provides round-trip verification.
>
...
> extracted data after success. About 4 GiB of free space is needed.
>
...
> Existing-archive updates, encryption, macOS metadata and other formats are
